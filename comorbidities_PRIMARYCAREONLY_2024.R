...
for (i in comorbids) {
  ...
  ### DISABLES - HES LINKAGE ONLY UP UNTIL MARCH 2023 ###
...
# DISABLED - HES LINKAGE ONLY UP UNTIL MARCH 2023
...
# 
# 
# ## Add to beginning of list so we don't have to remake interim tables when adding new comorbidity to end of above list
...
#    filter(date>=min_dob & ((source=="gp" & date<=gp_ons_end_date) | ((source=="hes_icd10" | source=="hes_opcs4") & (is.na(gp_ons_death_date) | date<=gp_ons_death_date)))) %>%
    filter(date>=min_dob & ((source=="gp" & date<=gp_end_date))) %>%
...
for (i in comorbids) {
  ...
  if (!exists(index_date_merge_tablename)) {next} else {
...
comorbidities <- comorbidities %>% analysis$cached("comorbidities_2024_march", unique_indexes="patid")
